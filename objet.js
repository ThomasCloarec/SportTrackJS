...
CalculDistance.prototype.calculateDistance2PointsGPS = function (lat1, long1, lat2, long2) {
    const R = 6378.137;
...
CalculDistance.prototype.calculatePathDistance = function (activity) {
    let distance = 0;
    for (let i = 0; i < activity.data.length - 1; i++) {
        distance += CalculDistance.prototype.calculateDistance2PointsGPS(activity.data[i]['latitude'], activity.data[i]['longitude'], activity.data[i + 1]['latitude'], activity.data[i + 1]['longitude']);
    }
...
console.log(new CalculDistance().calculatePathDistance(activity))
